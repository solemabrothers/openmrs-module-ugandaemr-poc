...
patientqueue.createCompletePatientQueueDialog = function () {
    patientqueue.completeQueueDialog = emr.setupConfirmationDialog({
        ...
        actions: {
            confirm: function () {
                emr.getFragmentActionWithCallback('ugandaemrpoc', 'completePatientQueue', 'post', visit.buildVisitTypeAttributeParams(),
                    function (v) {
...
patientqueue.createReadMessageDialog = function () {
    patientqueue.readMessageDialog = emr.setupConfirmationDialog({
        ...
        actions: {
            confirm: function () {
                emr.getFragmentActionWithCallback('ugandaemrpoc', 'alerts', 'markAlertAsRead', patientqueue.buildAlertAttributeParams(),
                    function (v) {
                        jq('#read_message' + ' .icon-spin').css('display', 'inline-block').parent().addClass('disabled');
                        emr.navigateTo({ applicationUrl: emr.applyContextModel("ugandaemrpoc/clinicianDashboard.page?app=ugandaemr.findPatient")});
                    });
            },
            cancel: function () {
                patientqueue.readMessageDialog.close();
...
patientqueue.createReadMessageDialog = function () {
    patientqueue.createMessageDialog = emr.setupConfirmationDialog({
        selector: '#read_message',
        actions: {
            confirm: function () {
                emr.getFragmentActionWithCallback('ugandaemrpoc', 'alerts', 'markAlertAsRead', patientqueue.buildAlertAttributeParams(),
